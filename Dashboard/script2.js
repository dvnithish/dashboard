async function fetchDataAndDisplayCounts2() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/two');
        ...
        for (const department in departmentCounts) {
            const id = `2${department.toLowerCase()}`;
