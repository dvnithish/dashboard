async function fetchDataAndDisplayCounts3() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/three');
        ...
        for (const department in departmentCounts) {
            const id = `3${department.toLowerCase()}`;
