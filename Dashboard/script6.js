async function fetchDataAndDisplayCounts6() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/six');
        ...
        for (const department in departmentCounts) {
            const id = `6${department.toLowerCase()}`;
