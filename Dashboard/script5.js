async function fetchDataAndDisplayCounts5() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/five');
        ...
        for (const department in departmentCounts) {
            const id = `5${department.toLowerCase()}`;
