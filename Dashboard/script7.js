async function fetchDataAndDisplayCounts7() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/seven');
        ...
        for (const department in departmentCounts) {
            const id = `7${department.toLowerCase()}`;
