async function fetchDataAndDisplayCounts4() {
    try {
        const response = await fetch('http://192.168.0.153:8080/honda/four');
        ...
        for (const department in departmentCounts) {
            const id = `4${department.toLowerCase()}`;
